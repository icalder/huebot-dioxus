  ...
  outputs =
    ...
    flake-utils.lib.eachDefaultSystem (
      ...
      let
        ...
        makeBuild =
          ...
          let
            ...
            huebot = craneLib.buildPackage (
              ...
              // {
                ...
                buildPhase = ''
                  ...
                  dx bundle --platform web --release @server --target ${rustTarget} --features server --no-default-features
                '';
            ...
            buildWithTag =
              tag:
              hostPkgs.dockerTools.buildImage {
                name = "huebot";
                inherit tag;
      ...
      in
      {
        ...
        packages.docker-aarch64 =
          if aarch64 != null then
            aarch64.dockerImage
          else if system == "aarch64-linux" then
            native.dockerImage
          else
            null;
        packages.docker-x86_64 =
          if x86_64 != null then
            x86_64.dockerImage
        ...
        packages.docker-all = pkgs.linkFarm "huebot-docker-all" (
          pkgs.lib.filter (x: x.path != null) [
            {
              name = "huebot-x86_64.tar.gz";
              path =
                if system == "x86_64-linux" then
                  native.dockerImage
                else if x86_64 != null then
                  x86_64.dockerImage
                else
                  null;
            }
            {
              name = "huebot-aarch64.tar.gz";
              path =
                if system == "aarch64-linux" then
                  native.dockerImage
                else if aarch64 != null then
                  aarch64.dockerImage
                else
                  null;
            }
