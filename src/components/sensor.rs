...
pub fn Sensor(sensor: CompositeSensor) -> Element {
    ...
    let motion_history = {
        ...
        let mut points: Vec<HistoryPoint> = h.into_iter().map(|(t, v)| HistoryPoint { time: t, value: if v { 1.0 } else { 0.0 } }).collect();
    ...
    let temp_history = {
        ...
        let mut points: Vec<HistoryPoint> = h.into_iter().map(|(t, v)| HistoryPoint { time: t, value: v }).collect();
        if let Some(last) = points.last() { points.push(HistoryPoint { time: current_time, value: last.value }); }
        points
    };
    let light_history = {
        let h = sensor.light.as_ref().map(|l| &l.history).cloned().unwrap_or_default();
        let mut points: Vec<HistoryPoint> = h.into_iter().map(|(t, v)| HistoryPoint { time: t, value: v as f64 }).collect();
        if let Some(last) = points.last() { points.push(HistoryPoint { time: current_time, value: last.value }); }
        points
    };
    ...
    rsx! {
        Link {
            to: Route::Graphs { sensor_id: sensor.device_id.clone() },
            class: "contents",
            Pulsing {
                key: "{fingerprint}",
                trigger: fingerprint,
                ...
                if !sensor.enabled {
                    div { class: "absolute top-0 right-0 bg-gray-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-bl-md z-20 uppercase tracking-tighter", "Disabled" }
                }
                div {
                    ...
                    div {
                        ...
                        h3 { class: "text-base font-bold tracking-wide text-gray-600 dark:text-gray-300 truncate", "{sensor.name}" }
                    }
